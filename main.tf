module "system" {
  ...
  disk_free_docs              = "Because Kafka persists all data to disk, it is necessary to monitor the amount of free disk space available to Kafka. Kafka will fail should its disk become full, so it’s very important that you keep track of disk growth over time, and set alerts to inform administrators at an appropriate amount of time before disk space is all but used up.\nhttps://www.datadoghq.com/blog/monitoring-kafka-performance-metrics/"
  ...
  disk_free_severity         = var.is_hosted_service ? "minor" : "major"
  disk_io_wait_severity      = var.is_hosted_service ? "minor" : "major"
  memory_free_bytes_severity = var.is_hosted_service ? "minor" : "major"
}