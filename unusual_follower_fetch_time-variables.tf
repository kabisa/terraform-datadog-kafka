variable "unusual_follower_fetch_time_enabled" {
  type    = bool
  default = true
}

variable "unusual_follower_fetch_time_warning" {
  type    = number
  default = 800
}

variable "unusual_follower_fetch_time_critical" {
  type    = number
  default = 1000
}

variable "unusual_follower_fetch_time_evaluation_period" {
  type    = string
  default = "last_30m"
}

variable "unusual_follower_fetch_time_severity" {
  type    = string
  default = "minor"
}

variable "unusual_follower_fetch_time_note" {
  type    = string
  default = ""
}

variable "unusual_follower_fetch_time_docs" {
  type    = string
  default = <<EOFF
...
variable "unusual_follower_fetch_time_require_full_window" {
  type = bool
  default = true
}

variable "unusual_follower_fetch_time_priority" {
